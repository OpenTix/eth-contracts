...
describe("VenueMint", function () {
  ...
  function getRandomElements<T>(list: T[], numElements: number): T[] {
    if (numElements > list.length) {
      throw new Error(
        "Number of elements to select cannot be greater than the list length."
      );
    }
  ...
  describe("Core Functionality", function () {
    ...
    it("can create an nft", async () => {
      ...
      expect(
        await contract.balanceOfBatch([await contract.getAddress()], [0])
      ).to.eql([1n]);
    });
    ...
    it("can purchase single ticket", async () => {
      ...
      // create the event
      const tmp = await vendor_contract_instance.create_new_event(
        "test",
        address,
        1,
        0,
      ...
      const user_contract_instance =
        vendor_contract_instance.connect(userWallet);

      // buy the tickets (way too much money give here)
      const resp = await user_contract_instance.buy_tickets("test", [0], {
        value: ethers.parseEther("1"),
      });

      // check that the user wallet now owns the NFT
      expect(
        await user_contract_instance.balanceOfBatch([userAddress], [0])
      ).to.eql([1n]);
    });
    ...
    it("can create 100-1000 NFTs", async () => {
      ...
      for (let i = 1; i <= max; i++) {
        // create the events
        await contract.create_new_event(
          `test ${i * base}`,
          contract_address,
          i * base,
          0,
        ...
        expect(
          await contract.balanceOfBatch(
            Array(i * base).fill(contract_address),
            Array.from({ length: i * base }, (_, i) => i + total_so_far)
          )
        ).to.eql(Array(i * base).fill(1n));
        total_so_far += i * max;
      }
    });
    ...
    it("can get the ids of an event", async () => {
      ...
      const tmp = await contract.create_new_event(
        "test",
        "0xblahblah",
        5,
        0,
      ...
      expect((await contract.get_event_ids("test"))[0]).to.eql(
        Array(0n, 1n, 2n, 3n, 4n)
      );
    ...
    it("will update the available ids for an event", async () => {
      ...
      await user_contract_instance.buy_tickets("test", [0], {
        value: ethers.parseEther("1"),
      });
    ...
    describe("User to User Ticket Transfer", function () {
      it("properly transfers tickets from user to user", async () => {
        ...
        // make wallets
        const nftholderwallet = ethers.Wallet.createRandom().connect(
          ethers.provider
        );
        const nftholderaddress = await nftholderwallet.getAddress();
        const nftpurchaserwallet = ethers.Wallet.createRandom().connect(
          ethers.provider
        );
        ...
        const tmp = await holder_instance.create_new_event(
          "test",
          "cool beans",
          1,
          0,
        ...
        const resp = await holder_instance.buy_tickets("test", [0], {
          value: ethers.parseEther("1"),
        });
        const resp1 =
          await holder_instance.allow_user_to_user_ticket_transfer();
        ...
        const holderfilter =
          init_contract.filters.User_To_User_Transfer_Concluded(
            nftholderaddress,
            nftpurchaseraddress
          );
        ...
        const holder_balance_before = await ethers.provider.getBalance(
          nftholderaddress
        );
        ...
        expect(
          await purchaser_instance.buy_ticket_from_user(nftholderaddress, 0, {
            value: ethers.parseEther("1"),
          })
        )
          .to.emit(purchaser_instance, "User_To_User_Transfer_Concluded")
          .withArgs(nftholderaddress, nftpurchaseraddress);
        expect(
          await purchaser_instance.balanceOf(nftpurchaseraddress, 0)
        ).to.equal(1n);

        // make sure the seller got the correct amount of money
        const holder_balance_after = await ethers.provider.getBalance(
          nftholderaddress
        );
    ...
    describe("Vendor Payment Functionality", function () {
      ...
      it("properly pays the vendor", async () => {
        ...
        expect(vendor_balance_after - vendor_balance_before).is.equal(
          ticket_cost
        );
      });
      ...
      it("properly pays the vendor when batch buying multiple tickets", async () => {
        ...
        // buy the tickets (way too much money give here)
        const resp = await user_contract_instance.buy_tickets("test", [0, 1], {
          value: ethers.parseEther("1"),
        });
      ...
      it("properly pays the vendor when buying multiple tickets in individual transactions", async () => {
        ...
        const resp2 = await user_contract_instance.buy_tickets("test", [1], {
          value: ethers.parseEther("1"),
        });
      ...
      it("properly pays the vendor when multiple users buy a ticket", async () => {
        ...
        const userWallet2 = ethers.Wallet.createRandom().connect(
          ethers.provider
        );
        ...
        const user2_contract_instance =
          user_contract_instance.connect(userWallet2);

        const resp2 = await user2_contract_instance.buy_tickets("test", [1], {
          value: ethers.parseEther("1"),
        });
      ...
      it("properly pays the vendor when multiple users buy multiple tickets", async () => {
        ...
        const resp2 = await user2_contract_instance.buy_tickets(
          "test",
          [2, 3],
          { value: ethers.parseEther("1") }
        );
      ...
      it("doesn't try to double sell a ticket in a batch event", async () => {
        ...
        expect(
          user2_contract_instance.buy_tickets("test", [1, 2], {
            value: ethers.parseEther("1"),
          })
      ...
      it("should work great", async () => {
        ...
        for (let i = 0; i < number_vendors; i++) {
          ...
          for (let j = 0; j < number_events; j++) {
            ...
            await vendor_contract.create_new_event(
              event_name,
              `i am vendor ${i}`,
              number_tickets,
              0,
              Array(number_tickets).fill(ticket_cost)
            );
        ...
        let client_vendor_mapping = Array.from(
          { length: number_vendors },
          (_, i) => i + 1
        )
        ...
        for (let i = 0; i < number_clients; i++) {
          ...
          let events_index_array = getRandomElements(
            Array.from({ length: num_events }, (_, i) => i + 1).map(
              (num) => num - 1
            ),
            number_events_to_purchase_from
          );
          ...
          for (let j = 0; j < number_events_to_purchase_from; j++) {
            let event_data =
              vendor_data[client_vendor_mapping[i]][events_index_array[j]];
            //console.log(event_data);
            let num_tickets_to_purchase = genRandom(
              1,
              event_data["number_tickets"]
            );

            let tickets_to_purchase = [
              ...(
                await client_contract.get_event_ids(event_data["event_name"])
              )[0].slice(0, num_tickets_to_purchase),
            ];

            let local_cost =
              num_tickets_to_purchase * event_data["ticket_cost"];
            // update total for total money vendor should have
            total_bought_from_vendor[client_vendor_mapping[i]] += local_cost;

            await client_contract.buy_tickets(
              event_data["event_name"],
              tickets_to_purchase,
              { value: ethers.parseEther("1") + BigInt(local_cost) }
            );
        ...
        for (let i = 0; i < number_vendors; i++) {
          vendor_after_balance[i] = await ethers.provider.getBalance(
            await vendors[i].getAddress()
          );
        }
        ...
        for (let i = 0; i < clients.length; i++) {
          ...
          for (let j = 0; j < big_ol_list.length; j++) {
            expect(
              await contract.balanceOfBatch(
                Array(big_ol_list[j].length).fill(
                  await clients[i].getAddress()
                ),
                big_ol_list[j]
              )
            ).to.eql(Array(big_ol_list[j].length).fill(1n));
          }
        }
      });
    });
